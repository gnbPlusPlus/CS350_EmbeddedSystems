...
class TemperatureMachine(StateMachine):
    ...
    ##
    ##  off - nothing lit up
    ##  heat - only red LED fading in and out
    ##  cool - only blue LED fading in and out
    ##
    ...
    def updateLights(self):
        ...
        if self.heat.is_active:
            ...
            if temp < self.setPoint:
                ...
                redLight.blink(on_time=0.5, off_time=0.5, fade_in_time=1, fade_out_time=1, n=None, background=True)
            else:
                # Red LED stays solid once heating is complete
                redLight.value = 1.0
            ...
            if temp > self.setPoint:
                ...
                # Fade in and out repeatedly
                blueLight.blink(on_time=0.5, off_time=0.5, fade_in_time=1, fade_out_time=1, n=None, background=True)
            else:
                # Blue LED stays solid once cooling is complete
                blueLight.value = 1.0
    ...
    def setupSerialOutput(self):
        output = f"{self.current_state},{self.getFahrenheit():0.1f},{self.setPoint}\n"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            self.lcd_line_1 = datetime.now().strftime('%b %d  %H:%M:%S\n')
    
            ## Setup Display Line 2
            if(altCounter < 6):
                self.lcd_line_2 = f"Temp: {self.getFahrenheit():0.1f}F"
    
                altCounter = altCounter + 1
            else:
                self.lcd_line_2 = f"{self.current_state}, {self.setPoint}F"
            ...
            ## Update Display
            screen.updateScreen(self.lcd_line_1 + self.lcd_line_2)
